resource "grafana_dashboard" "sol_collector_apm" {
  config_json = jsonencode({
    title       = "SOL - APM Dashboard"
    uid         = "sol-collector-apm"
    tags        = ["premium", "apm", "SOL", "monitoring"]
    timezone    = "browser"
    editable    = true
    refresh     = "30s"
    schemaVersion = 39
    ...
    panels = [
      ...
      # BIG STAT: SOL Price (Gradient Background)
      {
        id = 2
        title = "SOL Price"
        type = "stat"
      ...
      # BIG STAT: Throughput (Data Collection Rate)
      {
        ...
        targets = [{
          ...
          expr = "rate(sol_collector_data_collection_total[5m]) * 60"
          refId = "A"
        }]
        fieldConfig = {
          defaults = {
      ...
      # GAUGE: API Success Rate (Premium Gauge)
      {
        ...
        targets = [{
          ...
          expr = "sum(rate(sol_collector_api_calls_total{status=\"success\"}[5m])) / sum(rate(sol_collector_api_calls_total[5m])) * 100"
          refId = "A"
        }]
        fieldConfig = {
          defaults = {
      ...
      # HTTP Request Rate (Modern Timeseries with Gradient)
      {
        ...
        targets = [{
          ...
          expr = "sum by(endpoint) (rate(sol_collector_http_requests_total[5m]))"
          refId = "A"
        ...
        targets = [
          {
            ...
            expr = "histogram_quantile(0.50, sum by(le) (rate(sol_collector_http_request_duration_seconds_bucket[5m])))"
            refId = "A"
          ...
          },
          {
            ...
            expr = "histogram_quantile(0.95, sum by(le) (rate(sol_collector_http_request_duration_seconds_bucket[5m])))"
            refId = "B"
            legendFormat = "p95"
          },
          {
            ...
            expr = "histogram_quantile(0.99, sum by(le) (rate(sol_collector_http_request_duration_seconds_bucket[5m])))"
            refId = "C"
            legendFormat = "p99"
          }
        ]
      ...
      # Data Collection Success vs Errors (Dual Axis)
      {
        ...
        targets = [
          ...
          },
          {
            ...
            expr = "rate(sol_collector_data_collection_errors_total[5m]) * 60"
            refId = "B"
            legendFormat = "❌ Errors"
          }
        ]
      ...
      # API Calls Distribution (Pie Chart)
      {
        ...
        targets = [{
          ...
          expr = "sum by(status) (increase(sol_collector_api_calls_total[5m]))"
          refId = "A"
          legendFormat = "{{status}}"
        }]
        fieldConfig = {
      ...
      # SOL Price Chart (Premium Styling)
      {
        id = 9
        title = "₿ SOL Price (USDT)"
        type = "timeseries"
...
output "dashboard_sol_collector_url" {
  value = "http://localhost:3000/d/$${grafana_dashboard.sol_collector_apm.uid}"
}