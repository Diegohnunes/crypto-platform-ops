resource "grafana_dashboard" "eth_collector_apm" {
  config_json = jsonencode({
    title       = "ETH - APM Dashboard"
    uid         = "eth-collector-apm"
    tags        = ["premium", "apm", "ETH", "monitoring"]
    timezone    = "browser"
    editable    = true
    refresh     = "30s"
    schemaVersion = 39
    ...
    panels = [
      ...
      # BIG STAT: ETH Price (Gradient Background)
      {
        id = 2
        title = "ETH Price"
        type = "stat"
      ...
      # BIG STAT: Throughput (Data Collection Rate)
      {
        ...
        targets = [{
          ...
          expr = "rate(eth_collector_data_collection_total[5m]) * 60"
          refId = "A"
        }]
        fieldConfig = {
          defaults = {
      ...
      # GAUGE: API Success Rate (Premium Gauge)
      {
        ...
        targets = [{
          ...
          expr = "sum(rate(eth_collector_api_calls_total{status=\"success\"}[5m])) / sum(rate(eth_collector_api_calls_total[5m])) * 100"
          refId = "A"
        }]
        fieldConfig = {
          defaults = {
      ...
      # HTTP Request Rate (Modern Timeseries with Gradient)
      {
        ...
        targets = [{
          ...
          expr = "sum by(endpoint) (rate(eth_collector_http_requests_total[5m]))"
          refId = "A"
        ...
        targets = [
          {
            ...
            expr = "histogram_quantile(0.50, sum by(le) (rate(eth_collector_http_request_duration_seconds_bucket[5m])))"
            refId = "A"
          ...
          },
          {
            ...
            expr = "histogram_quantile(0.95, sum by(le) (rate(eth_collector_http_request_duration_seconds_bucket[5m])))"
            refId = "B"
            legendFormat = "p95"
          },
          {
            ...
            expr = "histogram_quantile(0.99, sum by(le) (rate(eth_collector_http_request_duration_seconds_bucket[5m])))"
            refId = "C"
            legendFormat = "p99"
          }
        ]
      ...
      # Data Collection Success vs Errors (Dual Axis)
      {
        ...
        targets = [
          ...
          },
          {
            ...
            expr = "rate(eth_collector_data_collection_errors_total[5m]) * 60"
            refId = "B"
            legendFormat = "❌ Errors"
          }
        ]
      ...
      # API Calls Distribution (Pie Chart)
      {
        ...
        targets = [{
          ...
          expr = "sum by(status) (increase(eth_collector_api_calls_total[5m]))"
          refId = "A"
          legendFormat = "{{status}}"
        }]
        fieldConfig = {
      ...
      # ETH Price Chart (Premium Styling)
      {
        id = 9
...
output "dashboard_eth_collector_url" {
  value = "http://localhost:3000/d/$${grafana_dashboard.eth_collector_apm.uid}"
}