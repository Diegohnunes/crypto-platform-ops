resource "grafana_dashboard" "dot_collector_apm" {
  config_json = jsonencode({
    title       = "DOT - APM Dashboard"
    uid         = "dot-collector-apm"
    tags        = ["premium", "apm", "DOT", "monitoring"]
    timezone    = "browser"
    editable    = true
    refresh     = "30s"
    schemaVersion = 39
    ...
    panels = [
      ...
      # BIG STAT: DOT Price (Gradient Background)
      {
        id = 2
        title = "DOT Price"
        type = "stat"
      ...
      # BIG STAT: Throughput (Data Collection Rate)
      {
        ...
        targets = [{
          ...
          expr = "rate(dot_collector_data_collection_total[5m]) * 60"
          refId = "A"
        }]
        fieldConfig = {
          defaults = {
      ...
      # GAUGE: API Success Rate (Premium Gauge)
      {
        ...
        targets = [{
          ...
          expr = "sum(rate(dot_collector_api_calls_total{status=\"success\"}[5m])) / sum(rate(dot_collector_api_calls_total[5m])) * 100"
          refId = "A"
        }]
        fieldConfig = {
          defaults = {
      ...
      # HTTP Request Rate (Modern Timeseries with Gradient)
      {
        ...
        targets = [{
          ...
          expr = "sum by(endpoint) (rate(dot_collector_http_requests_total[5m]))"
          refId = "A"
        ...
        targets = [
          {
            ...
            expr = "histogram_quantile(0.50, sum by(le) (rate(dot_collector_http_request_duration_seconds_bucket[5m])))"
            refId = "A"
          ...
          },
          {
            ...
            expr = "histogram_quantile(0.95, sum by(le) (rate(dot_collector_http_request_duration_seconds_bucket[5m])))"
            refId = "B"
            legendFormat = "p95"
          },
          {
            ...
            expr = "histogram_quantile(0.99, sum by(le) (rate(dot_collector_http_request_duration_seconds_bucket[5m])))"
            refId = "C"
            legendFormat = "p99"
          }
        ]
      ...
      # Data Collection Success vs Errors (Dual Axis)
      {
        ...
        targets = [
          ...
          },
          {
            ...
            expr = "rate(dot_collector_data_collection_errors_total[5m]) * 60"
            refId = "B"
            legendFormat = "❌ Errors"
          }
        ]
      ...
      # API Calls Distribution (Pie Chart)
      {
        ...
        targets = [{
          ...
          expr = "sum by(status) (increase(dot_collector_api_calls_total[5m]))"
          refId = "A"
          legendFormat = "{{status}}"
        }]
        fieldConfig = {
      ...
      # DOT Price Chart (Premium Styling)
      {
        id = 9
        title = "₿ DOT Price (USDT)"
        type = "timeseries"
...
output "dashboard_dot_collector_url" {
  value = "http://localhost:3000/d/$${grafana_dashboard.dot_collector_apm.uid}"
}